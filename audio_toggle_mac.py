...
class AudioToggle(rumps.App):
    def __init__(self):
        icon_path = Path(__file__).resolve().parent / "icons" / "speaker_template.png"
        icon_loaded = False
        if icon_path.exists():
            try:
                from AppKit import NSImage
                test_image = NSImage.alloc().initByReferencingFile_(str(icon_path))
                if test_image and test_image.isValid():
